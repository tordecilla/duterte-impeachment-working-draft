...
const receiptRows = [...document.querySelectorAll('[data-receipt-row]')];
const receiptResult = document.querySelector('#receipt-result');
const receiptEmpty = document.querySelector('#receipt-empty');
const receiptBody = document.querySelector('#receipt-table tbody');
const receiptSortButtons = [...document.querySelectorAll('[data-receipt-sort]')];
let receiptSort = {key: 'source', direction: 'ascending'};
const filterReceipts = () => {
  if (!receiptRows.length) return;
  const needle = receiptSearch?.value.trim().toLocaleLowerCase() || '';
  let shown = 0;
  ...
  if (receiptResult) receiptResult.textContent = `Showing ${shown} of ${receiptRows.length} receipts`;
  if (receiptEmpty) receiptEmpty.hidden = shown !== 0;
};
receiptSearch?.addEventListener('input', filterReceipts);
const sortReceipts = (key, direction) => {
  if (!receiptBody) return;
  const factor = direction === 'ascending' ? 1 : -1;
  const sorted = [...receiptRows].sort((left, right) => {
    ...
    if (leftValue && !rightValue) return -1;
    if (key === 'amount') return factor * (Number(leftValue) - Number(rightValue));
    return factor * leftValue.localeCompare(rightValue, undefined, {numeric: true, sensitivity: 'base'});
  });
  sorted.forEach((row) => receiptBody.appendChild(row));
  receiptSort = {key, direction};
  document.querySelectorAll('[data-receipt-heading]').forEach((heading) => {
    const active = heading.dataset.receiptHeading === key;
    heading.removeAttribute('aria-sort');
    if (active) heading.setAttribute('aria-sort', direction);
    const mark = heading.querySelector('.receipt-sort-mark');
    if (mark) mark.textContent = active ? (direction === 'ascending' ? '↑' : '↓') : '';
  });
};
receiptSortButtons.forEach((button) => button.addEventListener('click', () => {
  const key = button.dataset.receiptSort;
  const direction = receiptSort.key === key && receiptSort.direction === 'ascending'
    ? 'descending'
    : 'ascending';
  sortReceipts(key, direction);
}));
